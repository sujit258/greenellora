...
// POST create new handicraft product (auth required)
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const product = await HandicraftProduct.create({
      slug,
      name,
      category,
      image,
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Failed to create handicraft product" },
      { status: 500 }
    );
  }
}
