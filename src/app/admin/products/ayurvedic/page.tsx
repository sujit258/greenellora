...
export default function AdminAyurvedicProducts() {
  const { showToast } = useAdmin();
  const [products, setProducts] = useState<AyurvedicProduct[]>([]);
  ...
  async function handleDelete(id: string) {
    ...
    try {
      const response = await fetch(`/api/ayurvedic-products/${id}`, {
        method: "DELETE",
      });
      const data = await response.json();
  ...
  return (
    <AdminLayout title="Ayurvedic Products">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900">Ayurvedic Products</h2>
            <p className="mt-1 text-slate-600">Manage your ayurvedic product inventory</p>
          </div>
          <Link
                  ...
                  <tr key={product._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/products/ayurvedic/${product._id}/edit`}
